...
class PPO(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(
            self.time_step * self.sent_max_len * self.embedding_dim, 256
        )
        ...
        self.vocab_save_path = (
            "/home/user02/zss/robot/RSSN_CF/MHCH_SSA/makeup/vocab.pkl"
        )
        with open(self.vocab_save_path, "rb") as fin:
            self.word_embedding.weight.data.copy_(
                torch.from_numpy(pkl.load(fin).embeddings)
            )
    ...
    def make_batch(self, mode="train"):
        ...
        s_batch, a_batch, r_batch, s_prime_batch, done_batch = (
            torch.tensor(self.data["s_lst"], dtype=torch.long),
    ...
    def simple_evaluate_batch(self, mode="test"):
        ...
        for i in range(batch_num):
            ...
            if index.shape[0] != 0:
                ...
                s_batch = torch.reshape(
                    s_batch,
                    shape=[-1, self.time_step * self.sent_max_len * self.embedding_dim],
                )
                a_batch = a[start:end]
                index = index[:, 0]
        ...
        print(
            "%s\tprecision: %f; recall: %f; F1: %f; accuracy: %f."
            % (mode, p, r, F1, acc)
        )
        ...
        for j in range(batch_num):
            ...
            for i in range(K_epoch):
                ...
                loss = F.smooth_l1_loss(
                    self.v(s_batch), td_target.detach()
    ...
    def get_gtt_score(self, label_list, pre_list, lamb=0.0):
        gtt_score_list_1 = []
        gtt_score_list_2 = []
        gtt_score_list_3 = []
        for pres, labels in zip(pre_list, label_list):
            gtt_score_list_1.append(
                self.golden_transfer_within_tolerance_exp(pres, labels, t=1, lamb=lamb)
            )
            gtt_score_list_2.append(
                self.golden_transfer_within_tolerance_exp(pres, labels, t=2, lamb=lamb)
            )
            gtt_score_list_3.append(
                self.golden_transfer_within_tolerance_exp(pres, labels, t=3, lamb=lamb)
            )
    ...
    def golden_transfer_within_tolerance_exp(
        self, pre_labels, true_labels, t=1, eps=1e-7, lamb=0
    ):
        if t <= 0:
